...
func main() {
	...
	case subcmdJet.getCmdName():
		if err := subcmdJet.parse(os.Args[2:]); err != nil {
			fmt.Printf("error in cmd %s: %+v", subcmdJet.getCmdName(), err)
			subcmdJet.printDefaults()
			os.Exit(1)
		}
	default:
		flag.PrintDefaults()
